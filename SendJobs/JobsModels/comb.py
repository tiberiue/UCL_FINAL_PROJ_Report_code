...
parser.add_argument('--lrratio', dest='lrratio', type=float,
                    required= '--train' in sys.argv,
                    help='lrratio')

parser.add_argument('--lroptimizer', dest='lroptimizer', type=float,
                    required= '--train' in sys.argv,
                    help='lroptimizer')

parser.add_argument('--nepochscommon', dest='nepochscommon', type=int,
                    required= '--train' in sys.argv,
                    help='nepochscommon')
...
if __name__ == "__main__":
    ...
    if args.weight == 0:
        f = open("{}src/configs/DifferentModels/config_{}_{}_lrratio{}_nepochs{}.yaml".format(PathToWorkArea, args.train, args.model, args.lrratio, args.nepochscommon), "w")
    if args.weight == 1:
        f = open("{}src/configs/DifferentModels/config_{}_{}_lrratio{}_nepochs{}_weight.yaml".format(PathToWorkArea, args.train, args.model, args.lrratio, args.nepochscommon), "w")
    ...
    ## adversary
    PathToModel = PathToWorkArea + "Models/" + "{}_lr{}_nepochs{}/".format(args.model, args.learningrate, args.nepochs)
    os.system("ls {}adv_*nepochadv{}*.pt | tail -n 1 > latest.txt".format(PathToModel, args.nepochsadv))
    ...
    f.write("adversary:\n")
    f.write("  path_to_adv_model_weights: \"{}\"\n".format(ckpt))
    f.write("\n")

    ## combined
    f.write("combined:\n")
    f.write("  lr_ratio: {}\n".format(args.lrratio))
    f.write("  lambda_parameter: {}\n".format(args.lambdaparam))
    f.write("  loss_parameter: {}\n".format(args.lossparam))
    f.write("  lr_optimizer: {}\n".format(args.lroptimizer))
    f.write("  n_epochs_common: {}\n".format(args.nepochscommon))
    f.write("  save_every_epoch: True\n")
    f.write("  path_to_save_combined: \"{}\"\n".format(PathToModel))
    f.write("\n")


    ## Script file
    if args.weight == 0:
        fsh = open("scripts/{}_{}_lrratio{}_nepochs{}.sh".format(args.train, args.model, args.lrratio, args.nepochscommon), "w")
    if args.weight == 1:
        fsh = open("scripts/{}_{}_lrratio{}_nepochs{}_weight.sh".format(args.train, args.model, args.lrratio, args.nepochscommon), "w")
    ...
    if args.weight == 0:
        fsh.write("python combined_train.py configs/DifferentModels/config_{}_{}_lrratio{}_nepochs{}.yaml".format(args.train, args.model, args.lrratio, args.nepochscommon))
    if args.weight == 1:
        fsh.write("python weight_combined_train.py configs/DifferentModels/config_{}_{}_lrratio{}_nepochs{}_weight.yaml".format(args.train, args.model, args.lrratio, args.nepochscommon))


    ## Submission file
    if args.weight == 0:
        fsubmit = open("scripts/{}_{}_lrratio{}_nepochs{}.sub".format(args.train, args.model, args.lrratio, args.nepochscommon), "w")
    if args.weight == 1:
        fsubmit = open("scripts/{}_{}_lrratio{}_nepochs{}_weight.sub".format(args.train, args.model, args.lrratio, args.nepochscommon), "w")

    fsubmit.write("# here goes your shell script\n")
    fsubmit.write("executable    = {}_{}_lrratio{}_nepochs{}.sh\n".format(args.train, args.model, args.lrratio, args.nepochscommon) if args.weight == 0 else "executable    = {}_{}_lrratio{}_nepochs{}_weight.sh\n".format(args.train, args.model, args.lrratio, args.nepochscommon))
    fsubmit.write("# here you specify where to put .log, .out and .err files\n")
    fsubmit.write("arguments               = $(ClusterId)$(ProcId)\n")
    fsubmit.write("output                  = debug/{}_{}_lrratio{}_nepochs{}_w{}.$(ClusterId).out\n".format(args.train, args.model, args.lrratio, args.nepochscommon, args.weight))
    fsubmit.write("error                   = debug/{}_{}_lrratio{}_nepochs{}_w{}.$(ClusterId).err\n".format(args.train, args.model, args.lrratio, args.nepochscommon, args.weight))
    fsubmit.write("log                     = debug/{}_{}_lrratio{}_nepochs{}_w{}.$(ClusterId).log\n".format(args.train, args.model, args.lrratio, args.nepochscommon, args.weight))
    ...
    fsubmit.write("combined_train.py\n" if args.weight == 0 else "weight_combined_train.py\n")
